...
based on `Ctrough`. Say we’d like `Ctrough` not to exceed an arbitrary
safety limit of 5 mg/L. The rule could be as follows: if `Ctrough` is
greater than 5, multiply the last dose by 0.75, otherwise continue with
the same dose. Let’s illustrate this simple rule using events. We will
use the first compartment model with absorption from the model library
for this purpose.
...
![](v13_events_files/figure-html/event_dose_adaptation_ctrough-1.png)
